...
export class ConfigParser {
  ...
  addPlugin(plugin: Plugin, legacyPluginUrl?: string | null) {
    const parsedConfig = this.parseConfig(this.repoConfig);
    parsedConfig.plugins ??= [];

    const pluginOrgRepo = plugin.uses[0]?.plugin;
    const identifiers = buildPluginIdentifiers({ pluginUrl: legacyPluginUrl, pluginOrgRepo });
    const existingPluginIndex = findPluginConfigIndex(parsedConfig.plugins, identifiers);

    if (existingPluginIndex >= 0) {
      const existingPlugin = parsedConfig.plugins[existingPluginIndex];
      existingPlugin.uses[0] = {
        ...existingPlugin.uses[0],
        ...plugin.uses[0],
        plugin: pluginOrgRepo,
      };
      parsedConfig.plugins[existingPluginIndex] = canonicalizePluginConfig(existingPlugin, pluginOrgRepo) as Plugin;
    } else {
      parsedConfig.plugins.push(canonicalizePluginConfig(plugin, pluginOrgRepo) as Plugin);
    }
  ...
  removePlugin(plugin: Plugin, legacyPluginUrl?: string | null) {
    const parsedConfig = this.parseConfig(this.repoConfig);
    ...
    const identifiers = buildPluginIdentifiers({
      pluginUrl: legacyPluginUrl,
      pluginOrgRepo: plugin.uses[0]?.plugin,
    });
    parsedConfig.plugins = removePluginConfig(parsedConfig.plugins, identifiers) as Plugin[];
    this.newConfigYml = YAML.stringify(parsedConfig);
    this.repoConfig = this.newConfigYml;
    this.saveConfig();
  }
